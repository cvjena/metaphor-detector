...
filelist = []
with open('splits/sch.txt', 'r') as f:
    for l in f:
...
for fname in tqdm(filelist):
    if not "ADJ_NOUN" in fname:
        continue
    fname_full = os.path.join("vector_pairs_sch", fname)
    fname_cut = fname.replace("_ADJ_NOUN", "")

    tokens = get_tokens(pj("/lvdata/compling/gerdracor/processed_sch",  fname_cut))
    with open(pj("./sentences_sch", fname_cut), "rb") as f:
        sentences = pickle.load(f)
    sentences = [0]+sentences

    with open(pj('vector_pairs_sch', fname), 'rb') as f:
